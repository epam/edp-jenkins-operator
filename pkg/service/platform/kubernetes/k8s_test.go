...
func TestK8SService_CreateJenkinsScript(t *testing.T) {
	...
	_, err := svc.CreateJenkinsScript("ns", "name", true)
	require.NoError(t, err)
...
func TestK8SService_GetExternalEndpointErr(t *testing.T) {
	...
	_, _, _, err := service.GetExternalEndpoint(namespace, name) //nolint:dogsled // the values are not relevant to the test.
	assert.Error(t, err)
	assert.Contains(t, err.Error(), "failed to get ingress: test")
...
func TestK8SService_IsDeploymentReadyErr(t *testing.T) {
	...
	deployment.On("Get", context.TODO(), instance.Name, metav1.GetOptions{}).Return(nil, fmt.Errorf("test"))

	service := K8SService{appsV1Client: appClient}

	_, err := service.IsDeploymentReady(&instance)
	assert.Error(t, err)
	assert.Contains(t, err.Error(), "failed to get deployments: test")
...
func TestK8SService_CreateSecretErr(t *testing.T) {
	...
	assert.Error(t, err)
	assert.Contains(t, err.Error(), "failed to set reference for Secret ns object")
}
...
func TestK8SService_CreateSecret(t *testing.T) {
	...
	assert.NoError(t, service.CreateSecret(instance, namespace, data))
}
...
func TestK8SService_AddVolumeToInitContainer_DeploymentGetErr(t *testing.T) {
	...
	appClient.On("Deployments", "").Return(deployment)
	deployment.On("Get", context.TODO(), instance.Name, metav1.GetOptions{}).
...
func TestK8SService_AddVolumeToInitContainer_SelectContainerErr(t *testing.T) {
	...
	deployment.On("Patch", context.TODO(), deploymentInstance.Name, types.StrategicMergePatchType, raw, metav1.PatchOptions{}).Return(nil, fmt.Errorf("test"))
...
func TestK8SService_GetConfigMapData_NotFound(t *testing.T) {
	...
	assert.Error(t, err)
	assert.Contains(t, err.Error(), "failed to find config map name in namespace ns")
}
